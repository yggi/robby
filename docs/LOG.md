...
sessions into the paragraph of `docs/HISTORY.md` they belong to and delete them
...
**`docs/LOG.md` was two documents.** It held a dated worklog on top of an eleven-part
...
rewritten as it ages or it stops being readable. Split: `docs/HISTORY.md` is the
...
The mechanism is `laborsim`'s, which replaced three verbatim `docs/log/`
...
**`docs/handoff-2026-08.md` was deleted rather than kept as a record.** Last
...
`docs/design/game/playtest.md`, better placed and cross-linked. What was left
...
`git show 2d8fec1:docs/handoff-2026-08.md`.

**The root is now `CLAUDE.md`, `README.md` and configuration.** Every other
surface moved to `docs/`. Two files stayed and the reason is mechanical rather
...
`docs/MEMORY.md` rather than a bare filename. That is not tidiness —
`src/docs.test.ts` only resolves a backticked path that carries a directory, so
...
`docs/META.md` gained one entry that this session actually earned — **a document
...
problem. And `docs/HISTORY.md` came out of its first draft at 275 lines —
...
Cards: [R-000] opened. `docs/BOARD.md`, `docs/NOTES.md` and `docs/META.md`
created; `docs/design/` seeded; `HANDOFF.md` distributed.
...
- **four worlds → five chapters.** `chapters` is
  ...
  `docs/MEMORY.md` did not, and "all four worlds" reads differently once you know
  there is a fifth on the menu. Now a thread in `docs/NOTES.md`, because the
...
tree, and no content page has crept into the `docs/MEMORY.md` index. Each of the
